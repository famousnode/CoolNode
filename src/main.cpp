...
#if defined(NDEBUG)
#error "coolnode cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "coolnode Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 1) return 300000 * COIN;
		
	int64_t nSubsidy;
	
	if( nHeight > 1 && nHeight <= 16000 ) {
	        nSubsidy = 1 * COIN;
	}else if( nHeight > 16000 && nHeight <= 38000 ) {
	        nSubsidy = 50 * COIN;
	}else if( nHeight > 38000 && nHeight <= 82000 ) {
	        nSubsidy = 100 * COIN;
	} else {
		nSubsidy = 50 * COIN;
	}
...
{
    RenameThread("coolnode-scriptch");
