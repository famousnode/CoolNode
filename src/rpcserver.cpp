...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop coolnode server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "coolnode server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* COOL features */
        {"coolnode", "masternode", &masternode, true, true, false},
        {"coolnode", "masternodelist", &masternodelist, true, true, false},
        {"coolnode", "mnbudget", &mnbudget, true, true, false},
        {"coolnode", "mnbudgetvoteraw", &mnbudgetvoteraw, true, true, false},
        {"coolnode", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"coolnode", "mnsync", &mnsync, true, true, false},
        {"coolnode", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
        {"coolnode", "Darksend", &Darksend, false, false, true}, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
    ...
    if (((mapArgs["-rpcpassword"] == "") ||
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
                                             _("To use vivaldid, or the -server option to coolnode-qt, you must set an rpcpassword in the configuration file:\n"
                                               "%s\n"
                                               ...
                                               "for example: alertnotify=echo %%s | mail -s \"coolnode Alert\" admin@foo.com\n"),
...
{
    return "> coolnode-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:27329/\n";
}
