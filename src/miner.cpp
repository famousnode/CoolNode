...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("VVLMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("VVLMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("VVLMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coolnode-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("VVLMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running VVLMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadVVLMiner() exception");
    } catch (...) {
        LogPrintf("ThreadVVLMiner() exception");
    }

    LogPrintf("ThreadVVLMiner exiting\n");
}
